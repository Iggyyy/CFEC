...
_CallableTransform = Callable[[np.ndarray], np.ndarray]
...
class Cadex(BaseExplainer):
    ...
    def _gradient_descent(self, x: tf.Variable) -> tf.Variable:
        ...
        #y_expected: np.ndarray[Any] = np.array([0, 1]) if y == 0 else np.array([1, 0])
        y_expected: np.ndarray = np.array([0, 1]) if y == 0 else np.array([1, 0])
        ...
        for _ in range(self._max_epochs):
            ...
            if self._get_predicted_class(corrected_input) == np.argmax(y_expected):
                cor_numpy = corrected_input.numpy()
                return corrected_input

    def _get_predicted_class(self, x: tf.Variable):
        return np.argmax(self._model.predict(x), axis=1)[0]

    def _get_gradient(self, x, y_true):
        
        y_true = tf.constant([y_true.tolist()])
    ...
    #def _inverse_transform_input(self, _x: tf.Variable) -> np.ndarray[np.float32]:
    def _inverse_transform_input(self, _x: tf.Variable) -> np.ndarray:
        #x: np.ndarray[np.float32] = _x.numpy()[0]
        x: np.ndarray = _x.numpy()[0]
        if self._inverse_transform is not None:
