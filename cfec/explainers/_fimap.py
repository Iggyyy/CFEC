...
#def _freeze_constraints_to_mask(layer_size: int, freeze_constraints: List[Freeze], dtype='float32') -> NDArray[np.float32]:
def _freeze_constraints_to_mask(layer_size: int, freeze_constraints: List[Freeze], dtype='float32') -> np.ndarray:
    ...
    for constraint in freeze_constraints:
        ...
        columns: np.ndarray = np.asarray(constraint.columns)
        mask[columns] = 1
    return mask
...
#def _get_freeze_mask(shape, constraints: List[Any], mapper: DataFrameMapper, dtype="float32") -> np.ndarray[np.float32]:
...
class MyL1Regularizer(tf.keras.regularizers.Regularizer):
    ...
    def __init__(self, l1: float):
        super(MyL1Regularizer, self).__init__()
        self.l1 = l1
...
def _fit_g_s(s, g, x, y, s_epochs, g_epochs, g_optimizer=None, s_optimizer=None):
    ...
    for epoch in tqdm(range(epochs), desc='Training s' if g is None else 'Traning g'):
...
class Fimap(BaseExplainer):
    ...
    def __init__(self, fimap_safe_load: bool, fimap_s_filepath: str, fimap_g_filepath: str, 
                tau: float = 0.1, l1: float = 0.001, l2: float = 0.01, constraints: Optional[List[Any]] = None,
        ...
        self._s_prediction: np.ndarray
        self._use_mapper = use_mapper
        self._original_columns: Optional[List[str]] = None


        self.fimap_g_filepath = fimap_g_filepath
        self.fimap_s_filepath = fimap_s_filepath
        
        self.fimap_safe_load = fimap_safe_load
        

        # Load pretrained s
        if fimap_safe_load and fimap_s_filepath:
            print('Loading pretrained s')
            self._s = tf.keras.models.load_model(fimap_s_filepath)
    ...
    def fit(self, x: pd.DataFrame, y: pd.Series, **kwargs) -> None:
        ...
        # load pretrained weights g
        if self.fimap_safe_load and self.fimap_g_filepath:
            self._g.load_weights(self.fimap_g_filepath)
        ...
        if not self.fimap_safe_load:
            self.save_model(self._s, 's_')
            self.save_model(self._g, 'g_')
    ...
    def save_model(self, model: tf.keras.Model, s_or_g_prefix: str, directory: str = './') -> None:
        if directory[-1] != '/':
            directory += '/'
        if self.fimap_s_filepath is None or self.fimap_g_filepath is None:
            print('Model will not be saved. Save prefix not specified. Please pass this parameter while creating the model')
            return

        if 's' in s_or_g_prefix:
            save_path = self.fimap_s_filepath
            tf.keras.models.save_model(model, save_path, overwrite=True)
        else:
            save_path = self.fimap_g_filepath 
            model.save_weights(save_path, overwrite=True)

        print(f'Fimap model succesfully saved: {save_path}')