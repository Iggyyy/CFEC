...
from ..constraints import ValueNominal, OneHot


#def _stack_continuous_nominal(c: np.ndarray[np.float32], n: np.ndarray[np.float32]) -> np.ndarray[np.float32]:
def _stack_continuous_nominal(c: np.ndarray, n: np.ndarray) -> np.ndarray:
    if len(c.shape) < 2:
        c = c.reshape(-1, 1)
    if len(n.shape) < 2:
        n = n.reshape(-1, 1)
...
class DataFrameMapper:
    ...
    #def inverse_transform(self, x: np.ndarray[np.float32]) -> pd.DataFrame:
    ...
    #def _transform_continuous(self, x: pd.DataFrame) -> np.ndarray[np.float32]:
    def _transform_continuous(self, x: pd.DataFrame) -> np.ndarray:
        x = x.drop(columns=self._nominal_columns)
    ...
    #def _transform_nominal(self, x: pd.DataFrame) -> np.ndarray[np.float32]:
    def _transform_nominal(self, x: pd.DataFrame) -> np.ndarray:
        if self._one_hot_columns:
